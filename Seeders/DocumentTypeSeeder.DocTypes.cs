...
public partial class DocumentTypeSeeder
{
        ...
        IDataType? blockGridDataType)
    {
        ...
        // Create Document Type with ListView collection for child content overview
        var docType = new ContentType(_shortStringHelper, -1)
        {
            ...
            AllowedAsRoot = true,
            ListView = Guid.Parse(Constants.DataTypes.Guids.ListViewContent),
...
<body>
    ...
    #region Page Document Type AllowedContentTypes

    private async Task CreateDetailDocTypes(CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var variantDetail = await CreateDetailDocType(isVariant: true);
        Context.AddDetailDocType(variantDetail);

        cancellationToken.ThrowIfCancellationRequested();
        var invariantDetail = await CreateDetailDocType(isVariant: false);
        Context.AddDetailDocType(invariantDetail);

        Logger.LogInformation("Created {Count} detail document types", Context.DetailDocTypes.Count);
    }

    private async Task ConfigurePageDocTypeAllowedChildren(CancellationToken cancellationToken)
    {
        // Pages can have Detail children — allow only Detail doc types as children
        var allowedChildren = new List<ContentTypeSort>();
        int sortOrder = 0;
        foreach (var detailType in Context.DetailDocTypes)
        {
            allowedChildren.Add(new ContentTypeSort(detailType.Key, sortOrder++, detailType.Alias));
        }
        ...
        Logger.LogInformation("Configured AllowedContentTypes on {Count} page document types", allPageDocTypes.Count);
    }

    #endregion

    #region Parent Document Types (Section & Category)

    private async Task CreateParentDocumentTypes(CancellationToken cancellationToken)
    {
        // "List View - Content" well-known GUID (Constants.DataTypes.Guids.ListViewContent)
        var listViewContentCollectionId = Guid.Parse(Constants.DataTypes.Guids.ListViewContent);

        // Gather all child doc types for Category's AllowedContentTypes
        var allChildDocTypes = Context.SimpleDocTypes
            .Concat(Context.MediumDocTypes)
            .Concat(Context.ComplexDocTypes)
            .ToList();

        // Create Category doc types first (Section references them)
        var categoryDocTypes = new List<IContentType>();

        // Variant category
        cancellationToken.ThrowIfCancellationRequested();
        var variantCategory = await CreateParentDocType(
            isVariant: true, parentKind: "Category", listViewContentCollectionId, allChildDocTypes);
        categoryDocTypes.Add(variantCategory);
        Context.AddCategoryDocType(variantCategory);

        // Invariant category
        cancellationToken.ThrowIfCancellationRequested();
        var invariantCategory = await CreateParentDocType(
            isVariant: false, parentKind: "Category", listViewContentCollectionId, allChildDocTypes);
        categoryDocTypes.Add(invariantCategory);
        Context.AddCategoryDocType(invariantCategory);

        // Create Section doc types (allow Category doc types as children)
        cancellationToken.ThrowIfCancellationRequested();
        var variantSection = await CreateParentDocType(
            isVariant: true, parentKind: "Section", listViewContentCollectionId, categoryDocTypes);
        Context.AddSectionDocType(variantSection);

        cancellationToken.ThrowIfCancellationRequested();
        var invariantSection = await CreateParentDocType(
            isVariant: false, parentKind: "Section", listViewContentCollectionId, categoryDocTypes);
        Context.AddSectionDocType(invariantSection);

        Logger.LogInformation("Created {Count} parent document types (Section + Category, variant + invariant)",
            Context.SectionDocTypes.Count + Context.CategoryDocTypes.Count);
    }

    private async Task<IContentType> CreateParentDocType(
        bool isVariant,
        string parentKind,
        Guid collectionId,
        List<IContentType> allowedChildTypes)
    {
        var prefix = GetPrefix(PrefixType.ParentDocType);
        var variantLabel = isVariant ? "Variant" : "Invariant";
        var alias = $"{prefix}{parentKind}{variantLabel}";
        var name = $"Test {variantLabel} {parentKind}";

        var template = await CreateTemplate(alias, name, "simple");

        var docType = new ContentType(_shortStringHelper, -1)
        {
            Alias = alias,
            Name = name,
            Icon = parentKind == "Section" ? "icon-layout-list color-blue" : "icon-categories color-orange",
            AllowedAsRoot = parentKind == "Section",
            ListView = collectionId,
            Variations = isVariant ? ContentVariation.Culture : ContentVariation.Nothing
        };

        // Set allowed child content types
        var allowedChildren = new List<ContentTypeSort>();
        int sortOrder = 0;
        foreach (var childType in allowedChildTypes)
        {
            allowedChildren.Add(new ContentTypeSort(childType.Key, sortOrder++, childType.Alias));
        }
        docType.AllowedContentTypes = allowedChildren;
    ...
    private async Task<IContentType> CreateDetailDocType(bool isVariant)
    {
        var prefix = GetPrefix(PrefixType.ParentDocType);
        var variantLabel = isVariant ? "Variant" : "Invariant";
        var alias = $"{prefix}Detail{variantLabel}";
        var name = $"Test {variantLabel} Detail";
